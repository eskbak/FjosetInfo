...
type Settings = {
  ...
  viewDurations?: { // per-view duration in seconds (5..600)
    dashboard?: number;
    news?: number;
    calendar?: number;
    drinksMenu?: number;
...
// (Strava removed) -- no Strava tokens or endpoints
...
function normalizeSettings(input: any, prev?: Settings): Settings {
  ...
  const out: Settings = {
    ...
    viewDurations: {
      dashboard: clampInt(input?.viewDurations?.dashboard ?? base.viewDurations?.dashboard, 5, 600, base.viewDurations?.dashboard ?? base.rotateSeconds),
      news: clampInt(input?.viewDurations?.news ?? base.viewDurations?.news, 5, 600, base.viewDurations?.news ?? base.rotateSeconds),
...
function validateSettings(s: Settings): string[] {
  ...
  if (s.viewDurations) {
    for (const [k, v] of Object.entries(s.viewDurations)) {
      const n = Math.round(Number(v));
      if (!Number.isFinite(n) || n < 5 || n > 600) errors.push(`viewDurations.${k} must be 5..600`);
    }
  }
  return errors;
}
